...
sub PlotGraphs
{
    ...
    {
    	my $cmd = "$BSUB -q $queue -J PCW.$$ -cwd $outdir -e PCW.$$.%J.stderr -o PCW.$$.%J.stdout -We 2:00 -R \"rusage[mem=5]\" -M 10 -n 1 \"$outdir/PostCompileWrapper.csh\"";
    	$logger->debug( "COMMAND: $cmd");
    	eval{
    		`$BSUB -q $queue -J PCW.$$ -cwd $outdir -e PCW.$$.%J.stderr -o PCW.$$.%J.stdout -We 2:00 -R "rusage[mem=5]" -M 10 -n 1 "$outdir/PostCompileWrapper.csh"`;
