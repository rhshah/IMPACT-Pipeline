...
sub PlotGraphs
{
    ...
    else
    {
    	...
    	eval{
			...
			`$BSUB -q $queue -cwd $outdir -w "done(PCW.$$)" -J NotifyPCW.$$ -e NotifyPCW.$$.%J.stderr -o NotifyPCW.$$.stat -We 0:59 -R "rusage[mem=2]" -M 4 -n 1 "$outdir/Notify.csh"`;	
 ...
 eval{
    ...
    if ($@) {
    	...
    	my $cmd = "$BSUB -q $queue -J PreparePDF.$$ -cwd $outdir -e PreparePDF.$$.%J.stderr -o PreparePDF.$$.%J.stdout -We 0:59 -R \"rusage[mem=5]\" -M 10 -n 1 \"$PERL $MetricsScript $basename $R\"";
    	$logger->debug( "COMMAND: $cmd");
    	eval{
